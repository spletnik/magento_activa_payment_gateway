 ...
 */
class Spletnisistemi_Activa_Block_Redirect extends Mage_Core_Block_Abstract {
    protected function _toHtml() {
        $activaModel = Mage::getModel('activa/activa');
        $form = new Varien_Data_Form();

        $form->setAction($activaModel->getActivaUrl())
            ->setId('activa')
            ->setName('activa')
            ->setMethod('POST')
            ->setCharSet('utf-8')
        ...
        $form->setValues($activaModel->generateRequestData());

        $html = '';
        $html .= $this->__('SPLETNISISTEMI_ACTIVA_PAYMENT_START');
        $html .= $form->toHtml();
